...
- COLMAP **4.1.1** で確認。3.x とはオプション名が違う（`--SiftExtraction.use_gpu` → `--FeatureExtraction.use_gpu`）
- Python 3.8以上。**python.org版を使うこと**（Unity同梱のものは `sqlite3` が欠けている）
- 画像と中間ファイルはこの機械に置く。開発機（Ultra 5）には置かない

### どこまでを開発機（Ultra 5）でやれるか

**手順1〜5（疎な点群まで）は開発機のCPUで動く。** 実測値:

| 工程 | 時間（286枚・1080p・14コア） |
|---|---|
| 特徴点抽出 | 0.7分 |
| 突き合わせ | 3.7分 |
| 三角測量 | 0.2分 |

**手順6（密な点群）だけがCUDA必須。** `patch_match_stereo` はCUDA無しのビルドには含まれておらず、Intel内蔵GPUでは動かない。遅いのではなく実行できない。

この手法で一番あやしいのは「ARCoreの姿勢が写真測量に耐えるか」で、その答えは手順5で出る。**検証は開発機、仕上げだけ5070機**という分担ができる。
...
**「視線軸まわりの回転」の行も確認する。** AR Foundationの姿勢は画面の向き、CPU画像はセンサーの向きなので、端末を縦に持って撮ると90度ずれる。`frames.csv` の `screen_orientation` 列から自動で判定するが、この列を書く前に撮ったCSVでは `--camera-roll 90` を手で指定する必要がある（列が無い場合はエラーで止まる）。
...
CUDA無しの機械では `--FeatureExtraction.use_gpu 0` を足す（次の突き合わせでは `--FeatureMatching.use_gpu 0`）。
...
**点がほとんど起きない — まず姿勢の回転を疑う**

実際にこれで踏んだ。症状の出方が独特なので覚えておくとよい。

- 画像は286枚すべて登録される（COLMAPは与えられた姿勢を検査しないため）
- 特徴点も対応点も潤沢にある（1枚6,168点、検証通過1,257ペア×平均973対応）
- **なのに3次元点だけが起きない**

原因は、姿勢が画面の向き・画像がセンサーの向きで90度ずれていたこと。切り分けは簡単で、4通り試して点の数を比べればよい。

```bat
for %r in (0 90 180 270) do python tools\frames_to_colmap.py %REC% --camera-roll %r --out %WORK%\sp%r
```
...
**それでも点が少ない**
`det(R)` を確認する。次に `colmap gui` で `%WORK%\sparse_tri` を開き、カメラの並びが実際に歩いた軌跡の形をしているか目で見る。
