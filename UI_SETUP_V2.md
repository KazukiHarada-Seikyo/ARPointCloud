...
屋外で容量が尽きたとき、その場で古い撮影を消せるようにする。

### 13-1. ボタンを作る

`Canvas / Record` を選び、**Ctrl+D** で複製。名前を **`DeleteButton`** にする。
（既存を複製するとサイズと文字がそのまま使えて早い）

`RectTransform`:

| 項目 | 値 |
|---|---|
| Anchor | 右下（min `1, 0` / max `1, 0`） |
| Pivot | `0.5, 0.5` |
| Pos X / Pos Y | `-160` / `530` |
| Width × Height | `200` × `64` |

`Record` を Pos Y `440` に置いたので、その真上に並ぶ。

### 13-2. 中身

- 子の `Text (TMP)` の文字を **`削除`** にする
- Image の Color を `FFFFFF40` くらい（目立たせない）

### 13-3. OnClick

**OnClick の中身を入れ替える**（複製したので `ToggleRecording` が入っている）。

1. 既存の行の左下「−」で消す
2. 「＋」で追加
3. `DebugUI` をドラッグ
4. 関数に **`FrameCapture → DeleteOldestRecording()`** を選ぶ

### 13-4. 動き

**取り消せないので2回押させる。**

- 1回目 … 「もう一度押すと削除します / rec_20260811_091935 / 写真286枚 / 85MB」
- 2回目（6秒以内） … 実行して「85MB 空きました」
- 6秒放置 … 解除される

**古い順**に1つずつ消える。屋外で容量が尽きたとき、
さっき撮ったものは残したいのが普通なので、この順にしてある。

> 録画中は消せない。押すと「録画中は消せません」と出る。
